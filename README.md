...
1. **台指期夜盤走勢圖目前完全抓不到資料 —— 還沒解決，需要你決定下一步**：
   ...
   **問題（2026-08-20 發現）**：正式排程第一次跑就失敗——`requests.get()`
   ...
   這一層調 Python 的 headers 沒用，因為問題不在 headers 內容）。如果真的
   是這樣，唯一能繞過的方法是換成真的無頭瀏覽器（例如 Playwright）去抓，
   而不是純 `requests.get()`——這會讓 collect-night-session.yml 從「跑
   `pip install requests` 幾秒鐘」變成要另外安裝一顆瀏覽器、跑更久（但
   還是免費、還是在 GitHub Actions 額度內）。這是架構層級的改動，還沒
   幫你做，等你確認要不要投入這個方向，或改找別的資料源。在解決之前，
   Part 1 的台指期夜盤走勢圖會持續是空的（頁面上會顯示「尚無夜盤資料點」）。
