...
function renderMacro(pkg) {
  ...
  $('#macro-panel').innerHTML = `<div class="market-strip"><span class="market-item">加權 <b class="${metricClass(twse.twii?.change)}">${esc(fmt(twse.twii?.price))} ${esc(signed(twse.twii?.change_pct, 2, '%'))}</b></span><a class="market-item" href="https://www.cmoney.tw/forum/futures/TXF1?s=p" target="_blank" rel="noopener noreferrer" aria-label="開啟 CMoney 台指期主連續期貨頁面" style="color:inherit; text-decoration:none;">夜盤 <b class="${metricClass(night.change)}">${esc(fmt(night.price, 0))} ${esc(signed(night.change_pct, 2, '%'))}</b></a></div><h3>三大法人對稱橫條</h3><div>${renderFlow('外資', inst.foreign, maxInst)}${renderFlow('投信', inst.trust, maxInst)}${renderFlow('自營商', inst.dealer, maxInst)}</div><h3>美股收盤</h3><div class="quote-grid">${indexCards || '<span class="flat">尚無美股資料</span>'}</div><h3>ADR / 關聯股</h3><div class="quote-grid quote-grid--adr">${adrCards || '<span class="flat">尚無 ADR 資料</span>'}</div>`;
}
