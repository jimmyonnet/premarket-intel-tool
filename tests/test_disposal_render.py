...
from scripts.fetch_disposal import parse_active, parse_one_away


def test_fetch_disposal_parser_three_scenarios():
    """
    Test parsing from HTML fixture covering:
    (a) 已出關: trading_days_left is None
    (b) 倒數今日: trading_days_left == "0"
    (c) 倒數 N 日: trading_days_left > 0 (e.g. "3天")
    """
    ...
    assert stock_b["trading_days_left"] in ("0", 0, "0天")

    # (c) 倒數 N 日 (3天 -> "3天")
    stock_c = next(r for r in active_rows if r["code"] == "2455")
    assert stock_c["trading_days_left"] == "3天"
...
def test_template_rendering_filters_and_hidden_comment():
    ...
    - Stocks with countdown 0 or N days are handled properly
    """
    ...
    disposal_data = {
        ...
        "currently_in_disposal": [
            ...
            # (c) trading_days_left = "3天"
            {"market": "市", "code": "2455", "name": "全新", "matching": "2分", "start_date": "08-24", "end_date": "08-28", "exit_date": "08-31", "trading_days_left": "3天", "reason": "第2次"},
        ]
    }
    ...
    # Verify stock (b) 3293 is rendered
