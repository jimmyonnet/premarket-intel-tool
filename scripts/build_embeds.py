...
def build_embeds():
    ...
    <script>
      (function() {
        var t = 'light';
        try {
          t = window.parent.document.documentElement.getAttribute('data-theme') || localStorage.getItem('pmit_theme') || localStorage.getItem('premarket.theme');
        } catch(e){}
        if (t !== 'dark') t = 'light';
    ...
    <style>
      ...
      .empty { padding: 24px 16px !important; color: var(--text-muted) !important; font-size: 12.5px !important; text-align: center !important; }
      ...
      (function(){
        function applyTheme(theme) {
          if (!theme) {
            ...
            if (!theme) {
              try {
                theme = localStorage.getItem('pmit_theme') || localStorage.getItem('premarket.theme');
              } catch(e){}
            }
          }
        ...
        function send(){
          try {
            ...
            var finalH = Math.max(100, Math.ceil(totalH));
            parent.postMessage({type:'embed-height', id: location.pathname.split('/').pop().replace('.html',''), height: finalH}, '*');
          } catch(e){}
        }

        // MutationObserver and ResizeObserver to catch table rendering and details expansion
        ...
        setTimeout(function() {
          ...
          if (app && app.innerHTML.indexOf('載入中') >= 0) {
            app.innerHTML = '<div class="empty">⚠️ 資料載入逾時 <button onclick="location.reload()" style="background:var(--btn-bg); color:var(--blue); border:1px solid var(--btn-border); border-radius:4px; padding:3px 8px; margin-left:6px; cursor:pointer; font-weight:700;">🔄 點此重試</button></div>';
            send();
          }
        }, 3000);
